            ...
            compilation.hooks.optimizeChunks.tap(
                ...
                chunks => {
                    ...
                    const getChunkSetsInGraph = memoize(() => {
                        ...
                        console.error("module对应的重复chunk", chunkSetsInGraph, singleChunkSets);
                    ...
                    const groupChunksByExports = module => {
                        ...
                        const tempRequest = module.rawRequest || module.request;
                        if(tempRequest && (tempRequest.indexOf("js-cookie") >= 0 || tempRequest.indexOf("common_____g") >= 0)) {
                            // debugger;
                        }

                        for (const chunk of chunkGraph.getModuleChunksIterable(module)) {
                            const key = exportsInfo.getUsageKey(chunk.runtime);
                            console.warn("groupChunksByExports", key);
                        ...
                        for (const module of compilation.modules) {
                            ...
                            for (const chunks of groupedChunks) {
                                if (chunks.length === 1) {
                                    ...
                                    if (!chunkSetsInGraph.has(chunksKey)) {
                                        chunkSetsInGraph.set(chunksKey, new Set(chunks));
                                        temp.set(module.rawRequest||module.request, new Set(chunks));
                                    }
                                }
                            }
                        }
                        console.error("module拿到对应的chunks集合chunkSetsInGraph", temp);
                        console.error("module拿到对应的chunks集合singleChunkSets", singleChunkSets);
                        return { chunkSetsInGraph, singleChunkSets };
                    });
                    ...
                    const addModuleToChunksInfoMap = (
                        ...
                        const moduleTempRequest = module.rawRequest || module.request;
                        const key =
                            cacheGroup.key +
                            (name
                                ? ` name:${name}`
                                : ` chunks:${moduleTempRequest+" " + keyToString(selectedChunksKey)}`);
                        // Add module to maps
                    ...
                    for(let module of compilation.modules) {
                        tempModules.push(module.rawRequest || module.request);
                    }
                    console.error("目前准备遍历的是", tempModules);
                    ...
                    for (const module of compilation.modules) {
                        ...
                        const getCombsByUsedExports = memoize(() => {
                            ...
                            // if(groupedByUsedExports.length >= 3) {
                            // 	debugger;
                            // }
                            for (const chunks of groupedByUsedExports) {
                                const chunksKey = getKey(chunks);
                                const combsTemp = getExportsCombinations(chunksKey);
                                for (const comb of combsTemp)
                                    set.add(comb);
                            }
                        ...
                        // debugger;
                        console.warn("/n\n");
                        console.warn("目前遍历的是", module.rawRequest || module.request);
                        console.error("开始以上面这个module开始所有cacheGroup的遍历");
                        ...
                        for (const cacheGroupSource of cacheGroups) {
                            ...
                            if(combs.size >= 5) {
                                console.info("目前的cacheGroup对应的chunks数据是", combs);
                                console.info("目前的cacheGroup对应的cacheGroup.minChunks是", cacheGroup.minChunks);
                            }
                            for (const chunkCombination of combs) {
                                ...
                                if(combs.size >= 5) {
                                    console.info("cacheGroup.chunksFilter", cacheGroup);
                                    console.info("selectedChunks", selectedChunks);
                                    console.info("selectedChunksKey", selectedChunksKey);

                                    console.error("addModuleToChunksInfoMap执行完毕，进行下一个cacheGroup");
                                    console.info("\n");
                                }
                    ...
                    // Minimum size reduction to the main chunk (bundle), in bytes, needed for a chunk to be generated.
                    // Meaning if splitting into a chunk does not reduce the size of the main chunk (bundle) by the given amount of bytes,
                    // it won't be split, even if it meets the splitChunks.minSize value.
                    ...
                    console.error("开始分组", chunksInfoMap);
                    while (chunksInfoMap.size > 0) {
                        console.error("chunksInfoMap", chunksInfoMap);
                        debugger;
                        ...
                        for (const [key, info] of chunksInfoMap) {
                            if (isOverlap(info.chunks, usedChunks)) {
                                console.info("目前满足题意要删除的cacheGroup的chunks:", info);
                                console.warn("目前满足题意要删除的cacheGroup的chunks:", key);
                                console.info("目前newChunk的modules:", item.modules);
                        ...
                        console.warn("形成的newChunk", newChunk.name, newChunk);
                    }
                    ...
                    console.error("经过整理后的chunks是", compilation.chunks);
