...
const visitModules = (
    ...
    const processBlock = (block, isSrc) => {
        ...
        if (blockModules !== undefined) {
            ...
            if (queueBuffer.length > 0) {
                for (let i = queueBuffer.length - 1; i >= 0; i--) {
                    ...
                    if (queueBuffer[i].module && queueBuffer[i].module.rawRequest) {
                        queueBufferTemp.push(queueBuffer[i].module.rawRequest);
                    }
                }
        ...
        if (isSrc) {
            console.error(block.rawRequest||block.request, "处理同步依赖结束，准备处理它的异步依赖", queueBufferTemp);
        }


        // 开始处理block.rawRequest的异步依赖
        // 然后将关系放入到queueConnect中
        for (const b of block.blocks) {
            iteratorBlock(b);
        }
        ...
        if (block.blocks.length > 0 && module !== block) {
            ...
            console.error("处理异步依赖的异步依赖block.blocks", blocksWithNestedBlocks);
        }
    };
    ...
    const processQueue = () => {
        ...
        console.warn("queue处理所有同步依赖结束，准备处理queueConnect以及合并最小可用模块miniAvailable");
    };
    ...
    const calculateResultingAvailableModules = chunkGroupInfo => {
        ...
        console.error("processConnectQueue calculateResultingAvailableModules", temp);
    ...
    const processOutdatedChunkGroupInfo = () => {
        ...
        var tempoutdatedChunkGroupInfo = [];
        for (const info of outdatedChunkGroupInfo) {
            let temp = [];
            for (const module of info.minAvailableModules) {
                if (module.rawRequest && getIsSrc(module)) {
                    temp.push(module.rawRequest);
                }
            }
            tempoutdatedChunkGroupInfo.push(temp);
        }
        console.error("processOutdatedChunkGroupInfo", tempoutdatedChunkGroupInfo);
